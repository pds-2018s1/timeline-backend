...
describe('Model - Match', () => {
    ...
    describe ('save()', async () => {
        it('debe requerir size', async () => {
            try {
              await new Match({name:'nueva partida', timeline: [], players: [], deck:[]}).save()
              throw new Error('Tendría que haber fallado')
            } catch (err) {
              expect(err.message)
                .toEqual('Match validation failed: size: Path `size` is required.')
            }
          })
      ...
      describe('delete()', async () =>{
          it('debe eliminar el elemento indicado por el id', async() => {
              const saved = await new Match({name:'nueva partida', timeline: [], players: [], deck:[], size: 2}).save()
